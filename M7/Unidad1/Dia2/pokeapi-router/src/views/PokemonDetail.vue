<template>
  <div>
      ...
      <div id="pokedex">
        <div id="left">
          ...
          <div id="screen">
            ...
            <div id="picture">
              <img :src="pokemonDetail.image" alt="psykokwak" height="170" />
            </div>
        ...
        <div id="right">
          <div id="stats">
            <strong>Name:</strong> {{ pokemonDetail.name }}<br />
            <strong>Type:</strong> {{ pokemonDetail.type }}<br />
            <strong>Height:</strong>{{ pokemonDetail.height }}<br />
            <strong>Weight:</strong> {{ pokemonDetail.weight }}.<br /><br />
            <!-- <strong>The duck Pokemon</strong><br />
            Uses mysterious powers to perform various attacks. -->
          </div>
          <div id="blueButtons1">
            <div class="blueButton">1</div>
            <div class="blueButton">2</div>
            <div class="blueButton">3</div>
          ...
          <router-link :to="`/pokemon/${pokemonDetail.id - 1}`">
            <div id="yellowBox1">Anterior</div>
          </router-link>
          <router-link :to="`/pokemon/${pokemonDetail.id + 1}`">
            <div id="yellowBox2">Siguiente</div>
          </router-link>
    ...
    <div class="loader" v-show="isLoading">
      <img src="https://i.imgur.com/PqsuBCB.gif" alt="" />
    </div>
  </div>
</template>
...
export default {
  ...
  watch: {
    "$route.params.id": {
      async handler(newId) {
        // const currentId = parseInt(this.$route.params.id); //Busco el id en la url apenas cambia su valor
        const formattedPokemon = await this.getPokemonDetail(newId); //ejecuto un método que busca la info del pokemon
        this.pokemonDetail = formattedPokemon; // asigno el valor de la respuesta a mi variable de estado
      },
    },
  },

  async mounted() {
    const currentId = parseInt(this.$route.params.id); //Busco el id en la url apenas ingreso a la página
    const formattedPokemon = await this.getPokemonDetail(currentId); //ejecuto un método que busca la info del pokemon
    this.pokemonDetail = formattedPokemon; // asigno el valor de la respuesta a mi variable de estado
  },
};
</script>
